...
var app = new Vue({
    ...
    methods: {
        ...
        getNewCode: function () {
            fetch(url, {
                ...
                .then(json => {
                    ...
                    app.total = app.calcularm(app.miembros, "D") + app.calcularm(app.miembros, "R") + app.calcularm(app.miembros, "I");
                    app.total_average = app.calcularptotal(app.miembros);
                    app.most_loyal = app.primeros(app.miembros, app.diezporciento, "perdido", "menores");   
                    app.most_engaged = app.primeros(app.miembros, app.diezporciento, "voto", "menores");
                    app.least_engaged = app.primeros(app.miembros, app.diezporciento, "voto", "mayores");
                    app.least_loyal = app.primeros(app.miembros, app.diezporciento, "perdido", "mayores");

                })

        },

        caldiezporciento: function () {
            let diezpor = 0;
            diezpor = app.diezporciento = Math.round(app.miembros.length * 10 / 100);
            return diezpor;
        },
        ...
        calcularm: function (array, letraPartido) {
        ...
        primeros: function (arraymiembros, porcentaje, votos, mayomen) {
            let menores = [];
            let contador = porcentaje;
            let ordenados = [];

            if (mayomen == "menores") {
                ordenados = app.ordenarmenormayor(arraymiembros, votos);
            } else {
                ordenados = app.ordenarmayormenor(arraymiembros, votos);
            }
        ...
        ordenarmenormayor: function (arrayorden, votos) {
        ...
        //-------------------------------------------------------------------------------------------------------//
        ordenarmayormenor: function (arrayorden, votos) {
            let ordenados = arrayorden.sort(function (a, b) {
                if (votos == "perdido") {
                    if (a.votes_with_party_pct < b.votes_with_party_pct) {
                        return 1
